...
const ManageCloudRunInputSchema = {
  ...
  serverConfig: z.object({
    ...
    VpcConf: z.object({
      VpcId: z.string().describe('VPC网络ID，格式如 vpc-xxxxxxxx。必须与目标数据库/Redis 处于同一地域，并优先选择同一 VPC。强烈建议在服务首次创建时传入；已存在服务更换 VPC 通常需要先删除再重建'),
      SubnetId: z.string().describe('子网ID，格式如 subnet-xxxxxxxx。云托管实例将占用该子网 IP，需确保有足够可用 IP'),
    }).optional().describe('VPC网络配置（实例出网/私有网络）。用于让云托管实例接入指定 VPC，从而内网访问 MySQL/PostgreSQL/Redis/CVM 等资源。与 OpenAccessTypes（外部如何访问本服务）是不同概念。TCP 连库场景必须配置。MCP 会在首次创建时将 VpcConf 映射为 SDK vpcInfo(CreateType=2)；对已存在服务，更新路径可能无法更换 VPC，需 delete 后带正确 VpcConf 重新 deploy，并用 queryCloudRun detail 复核 ServerConfig.VpcConf'),
